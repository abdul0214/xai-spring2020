""" This is the main module for CLEAR. CLEAR takes the user-specified parameters 
 ...
 """
def run_Clear(dataset,first_obs,last_obs):
    ...
    np.random.seed(1)
    CLEAR_settings.init(dataset,first_obs,last_obs)

    (X_train, X_test_sample, model, numeric_features, category_prefix, feature_list,y_test_sample) \
    ...
    for neighbour_seed in range(0, CLEAR_settings.num_iterations):
        ...
        if not CLEAR_settings.LIME_comparison:
            if CLEAR_settings.multi_class:
                ...
                return (results_df,time_consumed,[X_test_sample,y_test_sample])
            ...
            if CLEAR_settings.first_obs == CLEAR_settings.last_obs:
                coefs2=CLEAR_perturbations.Single_prediction_report(results_df, nncomp_df, single_regress, explainer)
        else:
