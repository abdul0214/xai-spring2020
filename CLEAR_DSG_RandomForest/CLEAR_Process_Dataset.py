...
def Create_Numeric_Multi_Datasets():
 ...
 #   model = sklearn.ensemble.RandomForestClassifier(n_estimators=500)
    ...
    return X_train, X_test_sample, model, numeric_features, category_prefix, feature_list,[X_test,y_]
...
def Create_BreastC_Datasets():
    ...
    category_prefix = []
    y = df['diagnosis']
    (X_train, X_test_sample, model,y_test_sample) \
        =Create_Dataset(df, X, y, feature_list, numeric_features)
...
def Create_Dataset(df, X, y, feature_list, numeric_features):
    ...
    X_test_sample = X_test_sample.copy(deep=True)
    X_test_sample2 = X_test_sample2.copy(deep=True)
    y_test_sample = y_test_sample.copy(deep=True)
    y_test_sample2 = y_test_sample2.copy(deep=True)

    if CLEAR_settings.test_sample == 2:
        X_test_sample = X_test_sample2
        y_test_sample = y_test_sample2
    X_test_sample.reset_index(inplace=True, drop=True)
    y_test_sample.reset_index(inplace=True, drop=True)
    if CLEAR_settings.use_sklearn == True:
        ...
        model = sklearn.ensemble.RandomForestClassifier(n_estimators = 10, criterion = 'entropy', random_state = 0)
        model.fit(X_train, y_train)
        print('Accuracy statistics for Random Forest to be explained \n')
        print(sklearn.metrics.accuracy_score(y_test, model.predict(X_test)))
    else:
        print( "CLEAR_settings.use_sklearn: ", CLEAR_settings.use_sklearn)
        ...
        hparams = tf.contrib.tensor_forest.python.tensor_forest.ForestHParams(
                            num_trees=3, max_nodes=1000, num_classes=2, num_features=len(feature_list),regression=True)

        model = TensorForestEstimator(hparams)
    ...
    return (X_train, X_test_sample, model,y_test_sample)
