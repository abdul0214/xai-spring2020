""" This is the main module for CLEAR. CLEAR takes the user-specified parameters 
 ...
 """
def run_Clear(dataset,obs_no):


    import sys
    sys.path.append("C:/Users/wahab/Töölaud/XAI/XAI-SPRING2020/CLEAR/")
    ...
    np.random.seed(1)
    CLEAR_settings.init(case_study_input=dataset,obs_no=obs_no)
        ...
        if not CLEAR_settings.LIME_comparison:
            if CLEAR_settings.multi_class:
                ...
                for c in range(num_class):
                    ...
                    (nn_df, miss_df,coefs) = CLEAR_perturbations.Calculate_Perturbations(explainer, results_df, boundary_df, multi_index)
            ...
            if CLEAR_settings.first_obs == CLEAR_settings.last_obs:
                coefs=CLEAR_perturbations.Single_prediction_report(results_df, nncomp_df, single_regress, explainer)
                #print("weights : ",coefs )
        else:
